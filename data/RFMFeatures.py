...
class RFMFeatures:
    ...
    def custom_rfm(self, df):
        ...
        df['log_curr_dur_custody'] = np.log1p(df['curr_dur_custody'])
        df['dur_deviation_from_avg'] = df['curr_dur_custody'] - df['curr_dur_custody_mean']
        def is_increasing(row):
            history = row['curr_dur_custody_history']
            if len(history) == 0:
                return False
            return row['curr_dur_custody'] > history[-1]
        df['custody_increasing'] = df.apply(is_increasing, axis=1)
        df['arrest_speed_trend'] = df.groupby(self.id_col)['time_since_last_arrest'].diff().fillna(0)
        return df
